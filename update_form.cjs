...
// 1. Extract application section
const startTag = '<section class="application">';
const endTag = '  <style>'; // The style block comes right after it
const startIndex = content.indexOf(startTag);
let endIndex = content.indexOf('</section>', startIndex) + 10; 
// ensure we capture the whole section.
// let's just use regex to extract it
const applicationRegex = /<section class="application">[\s\S]*?<\/section>/;
const applicationMatch = content.match(applicationRegex);

if (!applicationMatch) {
    console.error("Could not find application section");
    process.exit(1);
}

let applicationSection = applicationMatch[0];

// Remove it from the original place
content = content.replace(applicationRegex, '');

// 2. Modify the application form internally
const newFormContent = `          <form class="application-form" id="whatsappForm">
            <div class="form-row">
              <input type="text" id="waName" placeholder="Adınız Soyadınız" required />
              <input type="email" id="waEmail" placeholder="E-posta Adresiniz" required />
            </div>
            <div class="form-row">
              <input type="tel" id="waPhone" placeholder="Telefon Numaranız" required />
              <input type="text" id="waCompany" placeholder="İşletme Adı" required />
            </div>
            <div class="form-row">
              <select id="waType" required>
                <option value="">İşletme Türü Seçin</option>
                <option value="Toptan">Toptan</option>
                <option value="Perakende">Perakende</option>
                <option value="Hem Toptan Hem Perakende">Hem Toptan Hem Perakende</option>
              </select>
              <select id="waSector" required>
                <option value="">Sektörünüzü Seçin</option>
                {sectors.map(s => <option value={s.name}>{s.name}</option>)}
              </select>
            </div>
            
            <div class="form-row" style="grid-template-columns: 1fr;">
                <select id="waExport" required onchange="document.getElementById('exportCountriesWrapper').style.display = this.value === 'Evet' ? 'block' : 'none'">
                    <option value="">İhracat Yapmak İstiyor musunuz?</option>
                    <option value="Evet">Evet</option>
                    <option value="Hayır">Hayır</option>
                </select>
            </div>
            
            <div class="form-row" id="exportCountriesWrapper" style="display: none; grid-template-columns: 1fr;">
                <label style="font-size: 0.9rem; color: #475569; margin-bottom: 0.5rem; display: block;">Hedef Ülkeleri Seçin (Birden fazla seçebilirsiniz)</label>
            ...
            <button type="submit" class="btn btn-primary btn-block">Hemen Başvurun</button>
          </form>
          <script>
            document.getElementById('whatsappForm').addEventListener('submit', function(e) {
              ...
              if (isExporting === 'Evet') {
                  const selectElement = document.getElementById('waExportCountries');
                  const selectedCountries = Array.from(selectElement.selectedOptions).map(opt => opt.value);
                  if(selectedCountries.length > 0) {
                      exportText += " (" + selectedCountries.join(', ') + ")";
                  } else {
...
applicationSection = applicationSection.replace(/<form class="application-form" id="whatsappForm">[\s\S]*?<\/script>/, newFormContent);

// 3. Insert application section right before advantages section
content = content.replace('  <!-- Avantajlar (6 kart) -->', applicationSection + '\\n\\n  <!-- Avantajlar (6 kart) -->');
